...
from features.methods.locators import recipes_section
import time
...
class Recipes(MainMethods):
    ...
    def __init__(self, driver):
        ...
        self.recipes = recipes_section
    
    # Open the recipes category
    def open_recipes(self):
        self.click_on_element(self.recipes.RECIPES_CATEGORY_BTN)
    ...
    def show_all_recipes(self):
        self.click_on_element(self.recipes.RECIPES_SEEALL_BTN)
        time.sleep(5)
    ...
    def type_recipe_name(self):
        self.click_on_element(self.recipes.RECIPES_SEARCH_INPUT)
        self.input(self.recipes.RECIPES_SEARCH_INPUT, "scrambled eggs")
        self.find_element(self.recipes.RECIPES_RESULTS_BOX)
    
    # Select first recipe by tapping on coordonates
    def select_first_recipe_entry(self):
        self.tap_coordonates(self.recipes.RECIPES_FIRST_RESULT_COORDONATES)
    ...
    def get_match_recipe(self):
        recipe_name = self.find_element_text(self.recipes.RECIPES_NAME_TEXT)
        recipe_availability = self.find_element_text(self.recipes.RECIPES_ENOUGH_INGREDIENTS_TEXT)
