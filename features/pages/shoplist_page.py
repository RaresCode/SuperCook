...
from features.methods.locators import shoplist_section
import time
...
class ShopList(MainMethods):
    ...
    def __init__(self, driver):
        ...
        self.shoplist = shoplist_section
    ...
    def click_shoplist(self):
        self.click_on_element(self.shoplist.SHOPLIST_CATEGORY_BTN)
        time.sleep(5)
    
    # Type ingredient name and click on it, wait for the result dropdown to load
    def type_shoplist_ingredient(self):
        self.find_element(self.shoplist.SHOPLIST_INPUT)
        self.click_on_element(self.shoplist.SHOPLIST_INPUT)
        self.input(self.shoplist.SHOPLIST_INPUT, "coconut milk")
        self.find_element(self.shoplist.SHOPLIST_RESULTS_BOX)
    
    # Select ingredient by coordonates
    def select_shoplist_ingredient(self):
        self.tap_coordonates(self.shoplist.SHOPLIST_FIRST_RESULT_COORDONATES)
    
    # Verify if the ingredient has been added to the list sucessfully
    def verify_shoplist_ingredient_existence(self):
        ingredient = self.find_element_text(self.shoplist.SHOPLIST_INLIST_BTN)
    ...
    def click_shoplist_ingredient(self):
        self.click_on_element(self.shoplist.SHOPLIST_INLIST_BTN)
    ...
    def verify_shoplist_ingredient_state(self):
        ingredient = self.find_element_text(self.shoplist.SHOPLIST_FINISHED_TEXT)
        if ingredient != "coconut milk":
            return "Not Found"
        else:
            return "Found"
